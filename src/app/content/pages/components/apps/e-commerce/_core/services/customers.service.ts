...
export class CustomersService {
	...
	findCustomers(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
		return environment.isMockEnabled ?
			this.findFakeCustomers(queryParams) :
			this.findRealCustomers(queryParams);
	}

	// Fake REST API (Mock)
	findFakeCustomers(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
	...
	findRealCustomers(queryParams: QueryParamsModel): Observable<QueryResultsModel> {
		// Note: Add headers if needed (tokens/bearer)
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	updateStatusForCustomer(customers: CustomerModel[], status: number): Observable<any> {
		return environment.isMockEnabled ?
			this.updateStatusForFakeCustomer(customers, status) :
			this.updateStatusForRealCustomer(customers, status);
	}

	// Fake REST API (Mock)
	// This code emulates server calls
	updateStatusForFakeCustomer(customers: CustomerModel[], status: number): Observable<any> {
		const tasks$ = [];
	...
	// Real REST API
	updateStatusForRealCustomer(customers: CustomerModel[], status: number): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		const body = {
	...
	deleteCustomers(ids: number[] = []): Observable<any> {
		 return environment.isMockEnabled ?
			this.deleteFakeCustomers(ids) :
			this.deleteRealCustomers(ids);
	}

	// Fake REST API (Mock)
	// This code emulates server calls
	deleteFakeCustomers(ids: number[] = []): Observable<any> {
		const tasks$ = [];
	...
	// Real REST API
	deleteRealCustomers(ids: number[] = []): Observable<any> {
